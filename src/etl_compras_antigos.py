...
class ETLComprasAntigos:
    ...
    def _corrigir_tipos(self, df):        
        ...
        for coluna in colunas_numericas:
            if coluna in df.columns:
                try:
                    ...
                    self.logger.warning(f"   ⚠️ {coluna}: erro na conversão numérica - {e}")
        ...
        if 'preco_unitario' in df.columns and 'qtd_itens_comprados' in df.columns:
            ...
            self.logger.warning("   ⚠️ Não foi possível calcular preco_total. Colunas ausentes.")
        ...
        if 'tipo_compra' in df.columns:
            ...
            self.logger.info("   ✅ tipo_compra: padronizado (ADMINISTRATIVA/JUDICIAL).")
        ...
        if 'codigo_br' in df.columns:
            ...
            self.logger.info("   ✅ codigo_br: prefixo 'BR' removido e padronizado.")
        ...
        if 'generico' in df.columns:
            ...
            self.logger.info("   ✅ generico: padronizado (SIM/NÃO) (Antigos anos).")
            
        # Colunas de texto (limpeza básica)
    ...
    def processar_todos_antigos(self):       
        ...
        for arquivo_path in arquivos:
            ...
            for encoding in encoding_tentativas:
                try:
                    ...
                    self.logger.warning(f"   ⚠️ Falha na leitura com encoding '{encoding}'. Tentando o próximo...")
        ...
        # ... (Resto do método de consolidação)
        
        # --- FIM DO LOOP, INÍCIO DA CONSOLIDAÇÃO ---
        
        if not dfs:
            self.logger.warning("\n⚠️ Nenhum DataFrame foi processado com sucesso para consolidar.")
            return None
...
if __name__ == "__main__":
    ...
    try:
        ...
        if df_antigos is not None:
            ...
            print("\n⚠️ O pipeline foi concluído, mas nenhum dado pôde ser consolidado.")
